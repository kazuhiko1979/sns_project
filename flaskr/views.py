...
def message(id):
    ...
    # 既に読まれていて、かつまだチェックしていない自分のメッセージをチェックする
...
def message_ajax():
    ...
    if not_read_messages_ids:
        ...
        db.session.commit()
    # 既に読まれた自分のメッセージでまだチェックしていないものを取得する
    not_checked_messages = Message.selelct_not_checked_messages(current_user.get_id(), user_id)
    not_checked_messages_ids = [not_checked_messages.id for not_checked_messages in not_checked_messages]
    if not_checked_messages_ids:
        with db.session.begin(subtransactions=True):
            Message.update_is_checked_by_ids(not_checked_messages_ids)
        db.session.commit()
    return jsonify(data=make_message_format(user, not_read_messages), checked_message_ids = not_checked_messages_ids)
